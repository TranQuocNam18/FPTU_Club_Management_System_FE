        ...
        <div className="px-6 pb-6">
          <div className="flex items-end gap-4 -mt-10 mb-4">
            ...
            <div className="flex-1 pb-2">
              ...
              <p className="text-sm text-slate-500 mt-0.5">{club.category} • Thành lập {formatDate(club.establishedDate)}</p>
            </div>
            {(isAdmin || user?.role === 'ClubManager') && (
              <Button icon={<Plus size={16} />} size="sm" onClick={() => setShowCreateEvent(true)}>Tạo sự kiện</Button>
            )}
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  {isManagerOrAdmin && <th className="px-6 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">Thao tác</th>}
                </tr>
              </thead>
                    ...
                    <tr key={m.id || m.userId} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium text-slate-800">{m.fullName || 'Thành viên'}</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              { label: 'Danh mục', value: club.category },
              { label: 'Trạng thái', value: club.status },
              { label: 'Ngày thành lập', value: formatDate(club.establishedDate) },
